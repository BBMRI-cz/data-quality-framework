<template>
  <ul class="pagination pagination-sm mt-2">
    <li class="page-item" :class="{ disabled: isInFirstPage }">
      <button class="page-link" @click="onClickPrevious">Prev</button>
    </li>

    <li class="page-item" :class="{ active: isInFirstPage }">
      <button class="page-link" @click="onClickFirst">1</button>
    </li>
    ...
    >
      <button class="page-link" :disabled="item.isDisabled" @click.prevent="onClickPage(item.page)">
        {{ item.page }}
      </button>
    </li>

    <li class="page-item" :class="{ active: isInLastPage }">
      <button class="page-link" @click="onClickLast">
        {{ totalPages }}
      </button>
    </li>
    <li class="page-item" :class="{ disabled: isInLastPage }">
      <button class="page-link" @click="onClickNext">Next</button>
    </li>
...
<script setup>
  ...
  const startPage = computed(() => {
    ...
    if (props.currentPage >= props.totalPages - half - 1)
