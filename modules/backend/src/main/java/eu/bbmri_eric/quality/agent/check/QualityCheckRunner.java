...
class QualityCheckRunner {
  ...
  QualityCheckRunner(CQLCheckRepository repository, ApplicationEventPublisher eventPublisher, FHIRStore fhirStore) {
  ...
  void onNewReport(NewReportEvent event) {
    ...
    for (CQLCheck check : checks) {
      ...
      eventPublisher.publishEvent(new CheckResultEvent(this, check.getId(),check.getName(), result.numberOfEntities(), null, LocalDateTime.now(), check.getWarningThreshold(), check.getErrorThreshold()));
    }
