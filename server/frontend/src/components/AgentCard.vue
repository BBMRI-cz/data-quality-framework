...
const latestReport = computed(() => {
  const agentReports = props.reports.filter(r =>
    (r.agentId || r.agent?.id) === props.agent.id
  )
...
const agentStatusTooltip = computed(() => {
  ...
  if (agentStats.value.warnings > 0) {
    parts.push(`${agentStats.value.warnings} warning${agentStats.value.warnings !== 1 ? 's' : ''}`)
  }
  ...
  return parts.join(', ') + ` out of ${totalChecks} check${totalChecks !== 1 ? 's' : ''}${categoryText}`
})
